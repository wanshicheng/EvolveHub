...
// 模拟数据
const mockUsers = [
  { id: 1, username: 'admin', nickname: '管理员', email: 'admin@evolve.com', phone: '13800138000', avatar: '', deptId: 1, status: 1, createTime: '2024-01-01 10:00:00' },
  { id: 2, username: 'user1', nickname: '用户一', email: 'user1@evolve.com', phone: '13800138001', avatar: '', deptId: 2, status: 1, createTime: '2024-01-02 10:00:00' },
  { id: 3, username: 'user2', nickname: '用户二', email: 'user2@evolve.com', phone: '13800138002', avatar: '', deptId: 2, status: 0, createTime: '2024-01-03 10:00:00' },
];
...
const mockRoles = [
  { id: 1, roleName: '超级管理员', roleKey: 'admin', roleCode: 'ROLE_ADMIN', dataScope: 1, sort: 1, status: 1, remark: '拥有所有权限', createTime: '2024-01-01' },
  { id: 2, roleName: '普通用户', roleKey: 'user', roleCode: 'ROLE_USER', dataScope: 3, sort: 2, status: 1, remark: '普通用户角色', createTime: '2024-01-02' },
  { id: 3, roleName: '部门管理员', roleKey: 'dept_admin', roleCode: 'ROLE_DEPT_ADMIN', dataScope: 2, sort: 3, status: 1, remark: '部门管理员角色', createTime: '2024-01-03' },
];

const mockPermissions = [
  { id: 1, parentId: 0, permName: '系统管理', permCode: 'system', permType: 'MENU', path: '/system', icon: 'settings', sort: 1, status: 1 },
  { id: 11, parentId: 1, permName: '用户管理', permCode: 'system:user', permType: 'MENU', path: '/system/user', icon: 'user', sort: 1, status: 1 },
  { id: 12, parentId: 1, permName: '角色管理', permCode: 'system:role', permType: 'MENU', path: '/system/role', icon: 'shield', sort: 2, status: 1 },
  { id: 13, parentId: 1, permName: '部门管理', permCode: 'system:dept', permType: 'MENU', path: '/system/dept', icon: 'building', sort: 3, status: 1 },
  { id: 14, parentId: 1, permName: '权限管理', permCode: 'system:permission', permType: 'MENU', path: '/system/permission', icon: 'lock', sort: 4, status: 1 },
  { id: 2, parentId: 0, permName: '订单管理', permCode: 'order', permType: 'MENU', path: '/order', icon: 'shopping-cart', sort: 2, status: 1 },
  { id: 21, parentId: 2, permName: '订单列表', permCode: 'order:list', permType: 'BUTTON', sort: 1, status: 1 },
  { id: 22, parentId: 2, permName: '订单详情', permCode: 'order:detail', permType: 'BUTTON', sort: 2, status: 1 },
];
...
export default defineEventHandler(async (event) => {
  ...
  if (path === '/api/dept/list' && method === 'get') {
    return { code: 0, data: { records: mockDepts, total: mockDepts.length, pageNum: 1, pageSize: 100 } };
  }
  ...
  if (path === '/api/permission/list' && method === 'get') {
    return { code: 0, data: { records: mockPermissions, total: mockPermissions.length, pageNum: 1, pageSize: 100 } };
  }
